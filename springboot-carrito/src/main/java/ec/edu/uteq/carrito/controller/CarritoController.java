...
public class CarritoController {
    ...
    private static final Map<String, Double> PRECIOS;
    static {
        Map<String, Double> catalogo = new LinkedHashMap<>();
        catalogo.put("Laptop", 450.0);
        catalogo.put("Mouse", 25.0);
        catalogo.put("Teclado", 45.0);
        catalogo.put("Monitor", 180.0);
        PRECIOS = Collections.unmodifiableMap(catalogo);
    }

    // Helper: obtener el carrito de la sesion (lo crea si no existe)
    ...
    public String agregar(@RequestParam String nombre, HttpSession session) {
        // Validar que el producto exista en el catalogo del servidor;
        // el precio nunca se toma del cliente.
