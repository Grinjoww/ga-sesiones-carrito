...
<body>
<h1>Tienda - Sesión PHP</h1>

<!-- Catálogo de productos -->
<h2>Catálogo</h2>
<form method="POST" action="agregar.php">
  <select name="nombre">
    <?php foreach (CATALOGO as $nombre => $precio): ?>
      <option value="<?= htmlspecialchars($nombre) ?>">
        <?= htmlspecialchars($nombre) ?> - $<?= $precio ?>
      </option>
    <?php endforeach; ?>
  </select>
...
<?php else: ?>
  <table>
    ...
    <tr>
      ...
      <td>$<?= htmlspecialchars((string) $item['precio']) ?></td>
      <td>
...
<!-- Debug: mostrar ID de sesión (solo en desarrollo) -->
